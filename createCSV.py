        ...
        for line in f:
            if line.startswith(filename + " was last updated"):
                ...
                lastUpdated = datetime.datetime.strptime(
                    line.split(" was last updated at ")[1].strip(),
                    "%Y-%m-%d %H:%M:%S.%f",
                )
                found = True
                break
...
def createMapping():
    ...
    for show in anime_list:
        ...
        if "anilist_id" in showKeys:
            ...
            if found:
                ...
                if aod_show["animeSeason"]:
                    # Check if the show has a year and a season before continuing
                    if (
                        "year" in aod_show["animeSeason"]
                        and "season" in aod_show["animeSeason"]
                    ):
                        ...
                        show["animeSeason"] = (
                            str(aod_show["animeSeason"]["year"])
                            + "-"
                            + str(aod_show["animeSeason"]["season"])
                        )
                    elif "year" in aod_show["animeSeason"]:
                        show["animeSeason"] = str(aod_show["animeSeason"]["year"])
                    else:
                        # Print the animeSeason if it's not a year and a season
                        print(
                            "Weird animeSeason for "
                            + aod_show["title"]
                            + ": "
                            + str(aod_show["animeSeason"])
                        )
                if "animeSeason" in show:
    ...
    print(
        "\nLinked "
        + str(len(mappings))
        + " shows and movies between anime-list and AOD"
    )
    return mappings
...
def orderSeasons(mappings):
    ...
    for i in range(len(mappings)):
        # Display progress
        sys.stdout.write(
            "\Putting seasons in order: " + str(i) + "/" + str(len(mappings))
        )
        ...
        if not any(d["id"] == mappings[i]["id"] for d in seasonedMappings):
            ...
            if len(seasons) > 1 and seasons[0]["type"] == "MOVIE":
                print(
                    "\nWARNING: That's a bit weird, "
                    + seasons[0]["title"]
                    + " ("
                    + str(seasons[0]["anilist_id"])
                    + ") is a movie and has "
                    + str(len(seasons))
                    + " seasons"
                )
    ...
    print(
        "\n\nFound "
        + str(len(seasonedMappings) - len(movies))
        + " seasons across "
        + str(len(shows))
        + " shows, and "
        + str(len(movies))
    ...
    for show in seasonedMappings:
        with open("mapping.csv", "a") as f:
            csv.writer(f, delimiter=";").writerow(
                [show["title"], show["anilist_id"], show["id"], show["animeSeason"]]
            )
            ...
            if not any(d[1] == str(show["anilist_id"]) for d in oldMapping):
                if show["type"] == "TV":
                    updates.append(
                        ...
                        + str(show["anilist_id"])
                        + ") Season "
                        + str(show["animeSeason"])
                    )
                elif show["type"] == "MOVIE":
                    updates.append(
                        "Added " + show["title"] + " (" + str(show["anilist_id"]) + ")"
                    )
